#!/bin/bash
# Run Case A (surface_flux) and Case B (volume_source) in sequence
set -e
...
run_case() {
    ...
    # Fix Windows line endings in all OpenFOAM text files
    find . -type f ! -path '*/polyMesh/*' | xargs -r sed -i 's/\r$//'

    echo "=== blockMesh ==="
    blockMesh > log.blockMesh 2>&1
    echo "blockMesh done, exit=$?"
    grep "cells:" log.blockMesh || true
    ...
    if [ -d postProcessing/volAverageT ]; then
        ...
        if [ -n "$VAT_DIR" ]; then
            tail -1 "$VAT_DIR/volFieldValue.dat" 2>/dev/null
        fi
    else
...
# Case A: surface flux (no topoSet needed)
run_case "/mnt/d/dev/SaunaFEM/results/dry_sauna_steady_surfflux" "Case A (surface_flux)" "no"

# Case B: volume source (needs topoSet for heaterZone)
run_case "/mnt/d/dev/SaunaFEM/results/dry_sauna_steady_volsource" "Case B (volume_source)" "yes"
