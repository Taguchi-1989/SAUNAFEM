...
### 10.4 体感温度モデル (Steadman 近似) の適用範囲外

**現状:** K-06 に Steadman の heat index 近似式を使用。

**物理的問題:** Steadman の式は気温 20-50℃ 程度の気象条件を前提としている。
サウナ環境（80-100℃、急激な湿度変化）では式の線形性が破綻し、
物理的に無意味な値を出力する可能性がある。

**今後の方針:** サウナ環境に特化した等価温度モデルの検討。
皮膚表面での結露・蒸発潜熱を考慮した熱ストレスモデル、
または WBGT の理論的拡張が候補。
...
ただし、人体が直接受ける輻射（ヒーターからの直達輻射）は
壁面経由ではなく直接的に体感温度に影響するため、
K-06（熱ストレス）の計算には別途ヒーター輻射の直達成分の
考慮が必要。
