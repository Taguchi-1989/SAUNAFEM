...
### 11.7 OpenFOAM 正確版の残課題

以下はレビュー指摘に基づく正確版 (OpenFOAM) の改善課題。
簡易版では対処済みだが、CFD 側には反映されていない項目。

**a) 輻射モデル (fvDOM/viewFactor) の未導入:**
ヒーター表面 (~300°C) からの輻射は $T^4$ 則に従い、対流と同等以上の
熱輸送を担う。現在の OpenFOAM 構成に輻射ソルバーがないため、
壁面温度分布と人体受熱量を過小評価する。
→ Phase 2 以降で `radiationModel fvDOM` または `viewFactor` を導入予定。

**b) 物性値の温度依存性 (`const` transport):**
Phase 1 では $\mu$, $\lambda$ を定数 (`const`) で扱っている。
サウナの温度域 (20-100°C) で動粘性係数は ~40% 変動する。
Phase 2 の `multiComponentMixture` では `sutherland` transport + `janaf` thermo
に移行するため、この問題は Phase 2 で自動的に解決される。

**c) 温度スキーム (linearUpwind vs TVD):**
`div(phi,T)` に `linearUpwind` を使用しているが、ヒーター近傍の急峻な温度勾配で
非物理的なオーバーシュートが生じるリスクがある。
`vanLeer` 等の TVD スキームへの変更を検討する。
ただし、linearUpwind + `bounded` で限定されているため実用上の問題は小さい。

**d) PIMPLE 外側反復回数:**
`nOuterCorrectors = 2` は浮力駆動流の密度-速度結合に対して不十分な可能性がある。
$Ra > 10^9$ の強い自然対流では 3-4 が推奨される。
計算コストとのトレードオフで、M0 メッシュでは 2 を維持し、
M2 以上の検証計算では 4 に増やす方針。

**e) 乱流-層流遷移:**
壁面下降流や床付近は低レイノルズ数で層流-遷移域にある。
SST k-omega は完全乱流を仮定するため、これらの領域で
乱流粘性を過大評価する可能性がある。
`kOmegaSSTLM` (Langtry-Menter 遷移モデル) の適用を検討。
