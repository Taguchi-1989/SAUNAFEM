...
\begin{itemize} \item $T_{\text{exhaust}}$: 排気口位置のゾーン温度（界面位置に応じて $T_{\text{upper}}$ または $T_{\text{lower}}$） \end{itemize}
...
\begin{itemize} \item $\dot{m}_{\text{vent}} \ge 0$: 自然換気では正（流入）方向のみ \end{itemize}
...
\hline
K-06 & 簡易熱ストレス指標 & 体感温度 (乾球+湿度補正) & \textbf{済} \\
\hline
...
\subsubsection{体感温度 (Steadman 近似)}
